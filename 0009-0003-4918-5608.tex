...
The model was trained on a curated corpus of 12,000 field-annotated images across the six classes, split into 70\% training ($N=8,400$), 15\% validation ($N=1,800$), and 15\% blind test ($N=1,800$) sets. Input images were resized to $224 \times 224 \times 3$ and normalized. Training was conducted using the Adam optimizer ($\beta_1=0.9, \beta_2=0.999$, weight decay $10^{-4}$) with an initial learning rate $\eta = 10^{-4}$ managed by cosine annealing over 50 epochs with a batch size of 32. Representative calibration for INT8 PTQ utilized 250 unlabelled plantation samples.
...
System~A was evaluated over commercial plantation blocks in Kurunegala, Sri Lanka. On regular $8.0\text{m}$ grid blocks ($N=1,250$ palms), the deterministic EDT algorithm achieved a precision of 97.2\%, recall of 95.8\%, and an overall F1-score of 96.5\%, operating without manual annotations. Minor false negatives occurred primarily in sectors with dense overlapping wild undergrowth or heavily leaning palm trunks.

In spatial anomaly profiling, the Moving-Window Local Spatial Z-Score engine identified 25 pre-symptomatic outlier palms experiencing mesophyll decay under multispectral NDVI ($Z < -2.0$), whereas global RGB thresholding flagged only 4 palms displaying visible chlorosis.

\subsection{Microscopic Classification Performance}
The quantized INT8 MobileNetV2 model was benchmarked against the blind test set of 1,800 field-captured images ($N=300$ per class). As reported in Table~\ref{tab:metrics}, the model achieved a macro-averaged precision of 93.8\%, recall of 93.3\%, and F1-score of 93.5\%, with a Cohen's Kappa coefficient of $\kappa = 0.9214$.

High visual contrast foliar conditions such as Healthy Leaves (96.5\% F1) and Gray Leaf Spot (95.4\% F1) demonstrated high discriminative accuracy. Conversely, classes involving trunk or root interface decay, such as Stem Bleeding (90.7\% F1) and Bud Root Dropping (89.4\% F1), exhibited lower recall due to fibrous bark texture variations and partial soil splatter in ground-level field photography.
...
\midrule
Bud Rot & 95.2\% & 94.3\% & 94.8\% & 300 \\
Stem Bleeding & 91.8\% & 89.7\% & 90.7\% & 300 \\
Leaf Rot & 93.5\% & 93.0\% & 93.2\% & 300 \\
Bud Root Dropping & 90.1\% & 88.7\% & 89.4\% & 300 \\
Gray Leaf Spot & 95.7\% & 95.0\% & 95.4\% & 300 \\
Healthy Leaves & 96.7\% & 96.3\% & 96.5\% & 300 \\
\midrule
\textbf{Macro Average} & \textbf{93.8\%} & \textbf{93.3\%} & \textbf{93.5\%} & \textbf{1,800} \\
\bottomrule
\end{tabular}%
}
\label{tab:metrics}
...
Model efficiency and hardware execution benchmarks across target platforms are detailed in Table~\ref{tab:hardware_benchmarks}. INT8 Post-Training Quantization reduced the model disk footprint from 10.84~MB to 2.71~MB (75.0\% compression) with a Top-1 accuracy trade-off of only 0.70\% (94.90\% FP32 vs. 94.20\% INT8 on validation data). Inference latency profiled on a budget MediaTek Helio G99 mobile CPU using the XNNPACK delegate averaged 68.2~ms per frame, ensuring responsive field execution.
...
Top-1 Accuracy & 94.90\% & 94.20\% & -0.70\% \\
