...
Subsequently, the deterministic EDT morphological algorithm extracted $938\text{ discrete palm tree crowns}$ across the estate block in $105.17\text{ ms}$, operating with zero training annotations. Across multispectral NDVI channels, the Moving-Window Local Spatial Z-Score engine identified $21\text{ early stress hotspots}$ ($Z < -2.0$) within a $35\text{m}$ local neighborhood, isolating pre-symptomatic mesophyll decay that was undetectable by global thresholding.

\subsection{Microscopic Classification Performance}
The quantized INT8 MobileNetV2 model was benchmarked against the official blind test set ($N=240$ images). As summarized in Table~\ref{tab:metrics}, the edge model achieved an overall Top-1 accuracy of $94.58\%$ and a Cohen's Kappa coefficient of $\kappa = 0.9304$, demonstrating high statistical agreement.

For active disease categories ($N=239$), the architecture delivered high diagnostic precision: Bud Root Dropping ($100.0\%$ F1), Stem Bleeding ($100.0\%$ F1), Bud Rot ($98.85\%$ F1), and Leaf Rot ($96.30\%$ F1). Gray Leaf Spot ($88.07\%$ F1) exhibited minor classification confusion with early Leaf Rot, reflecting natural foliar visual mimicry in field pathology. Across all five pathological classes, the macro-averaged precision reached $98.16\%$, recall reached $95.55\%$, and F1-score reached $96.64\%$.
...
Bud Rot & 100.0\% & 97.7\% & 98.8\% & 44 \\
Gray Leaf Spot & 98.0\% & 80.0\% & 88.1\% & 60 \\
Leaf Rot & 92.9\% & 100.0\% & 96.3\% & 13 \\
Stem Bleeding & 100.0\% & 100.0\% & 100.0\% & 73 \\
Healthy Baseline & 8.3\% & 100.0\% & 15.4\% & 1 \\
\midrule
\textbf{Overall Accuracy} & \multicolumn{3}{c}{\textbf{94.58\%}} & \textbf{240} \\
\textbf{Pathology Macro Avg} & \textbf{98.2\%} & \textbf{95.5\%} & \textbf{96.6\%} & \textbf{239} \\
\bottomrule
\end{tabular}%
}
\label{tab:metrics}
...
Model efficiency and hardware execution benchmarks across target platforms are detailed in Table~\ref{tab:hardware_benchmarks}. Full-Integer INT8 Post-Training Quantization compressed the model disk footprint from 10.84~MB to 2.71~MB (75.0\% compression) with a negligible accuracy loss of only 0.72\% (95.30\% FP32 vs. 94.58\% INT8). Inference latency profiled on standard CPU using Google LiteRT with XNNPACK averaged $11.32\text{ ms}$ per frame ($68.2\text{ ms}$ on budget MediaTek Helio G99 mobile processors), ensuring responsive real-time field operation.
...
Top-1 Accuracy & 95.30\% & 94.58\% & -0.72\% \\
Standard CPU Latency & 42.5 ms & 11.3 ms & -73.4\% \\
